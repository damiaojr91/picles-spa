...
const petSchema = z.object({
  name: z.string().min(2, 'Nome deve ter no mínimo 2 caracteres').max(64, 'Nome deve ter no máximo 64 caracteres'),
  type: z.string(),
  size: z.string(),
  gender: z.string(),
  bio: z.string().min(15, 'Sobre deve ter no mínimo 15 caracteres').max(1024, 'Sobre deve ter no máximo 1024 caracteres'),
});
      ...
      <form className={styles.container} onSubmit={handleSubmit(submit)}>
        ...
        <div className={styles.fields}>
          ...
          <div>
            ...
            />
            {formState.errors?.type && <p className={styles.formError}>{formState.errors?.type.message}</p>}
          </div>
          <div>
            <Select
              label="Sexo"
            ...
            />
            {formState.errors?.gender && <p className={styles.formError}>{formState.errors?.gender.message}</p>}
          </div>
          <div>
            <Select
              label="Porte"
            ...
            />
            {formState.errors?.size && <p className={styles.formError}>{formState.errors?.size.message}</p>}
          </div>
        </div>
        <TextArea label="Sobre" {...register('bio')} />
        {formState.errors?.bio && <p className={styles.formError}>{formState.errors?.bio.message}</p>}
